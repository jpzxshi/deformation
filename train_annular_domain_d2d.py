...
def plot(data, net):
    ...
    from tools import annular_2_circle
    import numpy as np
    ...
    X = annular_2_circle(points, outer[n_train + n], inner[n_train + n])
    y_true = raw_data['u_{}'.format(n_train + n)]
    y_pred = net.predict((domain, f, X), returnnp=True).reshape(-1)
    ...
    plt.savefig('prediction_annular_d2d.pdf')
...
def main():
    ...
    #### data
    path = './data/annular_d2d/'  # the directory of the dataset
    #### MIONet
    sizes = [
        [250] + [500] * 4 + [1000],
        [5000, -1000], # -1000 means the last layer is without bias
        [2] + [500] * 4 + [1000],
    ]
    activation = 'relu'
    #### training
